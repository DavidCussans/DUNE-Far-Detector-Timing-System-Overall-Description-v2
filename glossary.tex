...
\newduneabbrev{dp}{DP}{Dual-Phase}{Distinguishes one of the DUNE far detector technologies by the fact that it operates using argon %Distinguishes one of the four \SI{10}{\kton} \glspl{detmodule} of the DUNE far detector by the fact that it operates using argon 
 ...
 %vertical-drift
\newduneabbrev{vd}{VD}{Vertical-Drift}{Detector module with vertical drift}
 
  %vertical-drift
\newduneabbrev{hd}{HD}{Horizontal-Drift}{Detector module with horizontal drift}
 
 %bottom-detector-electronics
\newduneabbrev{bde}{BDE}{Bottom-Detector-Electronics}{The bottom readout electronics of the vertical drift module}

 %top-detector-electronics
\newduneabbrev{tde}{TDE}{Top-Detector-Electronics}{The top readout electronics of the vertical drift module}
...
\newduneword{dcsk}{DCSK}{Duty Cycle Shift Keying - states '0' and '1' are represented by different duty cycles. In the \gls{dts} a duty cycle of 25\%/75\% is used to represent 0/1. This keying method gives a rising edge at the beginning of every bit allowing a clock to be reconstructed with a standard PLL based clock generator without the addition of a \gls{cdr} device}

%front-end mother board
\newduneabbrev{femb}{FEMB}{front-end mother board}{Refers a unit of
  the \gls{hd} TPC cold electronics that contains the front-end amplifier
...
% TAI
\newduneabbrev{tai}{TAI}{International Atomic Time}{International Atomic Time is a high-precision atomic coordinate time standard.}
...
\newduneabbrev{hsi}{HSI}{Hardware Signal Interface}{Units receiving external signals into the \dword{dts}}
...
\newduneabbrev{wib}{WIB}{warm interface board}{Digital electronics
  situated just outside the cryostat that receives digital data
  from the \dwords{femb} over cold copper connections and sends it to the data acquisition system.}
...
\newduneabbrev{dts}{DTS}{Dune Timing System}{The timing and synchronization system for DUNE. An umbrella term encompassing both the \gls{hd} and \gls{vd} detector module timing systems}
...
\newduneword{ipbus}{IPBus}{IPBus UDP/IP based control bus. Firmware library that provides an A32/D32 bus inside the FPGA and software library to control from a PC.}


\newduneabbrev{dtsep}{endpoint}{DUNE Timing System Endpoint}{A point at which the \gls{dts} delivers timing and synchronization information. Timestamps and synchronization messages are delivered by a firmware block inside an FPGA. The phase aligned clock can either be delivered by a clock management unit inside an FPGA or by a PLL based clock generator.}
